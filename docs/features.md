...
- 连接卡片按状态展示：未连接显示状态、单个「连接地址」输入框和「连接」；已连接再显示地址、来源和「重启服务」「终止服务」（远程实例不显示「重启服务」）
...
- 点 **连接** 接受完整 HTTP(S) URL（含 `?token=…`）或 `主机:端口`，按完整地址探测并交给官方视图，不自动 spawn、不丢弃路径或查询参数；成功连接本机地址会写回启动端口
- 手动连接 URL 仅在当前会话保留，不将 token 写入 settings.json；重新检测和切换连接保留完整链接，地址摘要、连接失败提示和看门狗日志不展示查询参数。token 是否有效由官方页面验证
