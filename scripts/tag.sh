...
#   ./scripts/tag.sh beta 0.1.1      指定系列（已有 beta.10 则提议 beta.11）
#   ./scripts/tag.sh release         提议下一个正式版 vX.Y.(Z+1)
#   ./scripts/tag.sh release 0.1.2   指定正式版号
#
# 推 tag 后 GitHub release.yml 自动构建 DMG：
#   正式 tag（无 '-'）   → GitHub Release + 回写 main 的 package.json
#   预发布 tag（含 '-'） → GitHub pre-release，不动 main
set -euo pipefail
...
confirm_tag() {
  ...
  echo
  if [[ "$tag" == *-* ]]; then
    echo "GitHub pre-release（DMG），不回写 main 的 package.json"
  else
...
cmd_beta() {
  ...
  if [[ -z "$series" ]]; then
    if [[ -z "$latest" ]]; then
      echo "还没有正式版 tag，请指定系列： ./scripts/tag.sh beta 0.1.1" >&2
      exit 1
    fi
  ...
  # 正式版已发出则拒绝再打该系列 beta（避免 0.1.1 已发还出 0.1.1-beta.1）
...
cmd_release() {
  ...
  if [[ -z "$ver" ]]; then
    if [[ -z "$latest" ]]; then
      echo "还没有正式版 tag，请指定版本： ./scripts/tag.sh release 0.1.0" >&2
      exit 1
    fi
  ...
  echo "  提议：       ${tag}  （回写 main 的 package.json）"
